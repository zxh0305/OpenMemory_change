# 查看衰退分数完整指南 📊

## 🎯 衰退分数在哪里？

衰退分数存储在数据库的 `memories` 表中的 `decay_score` 字段。

---

## 💾 在数据库中查看

### 方法1：使用 SQLite 命令行

```bash
# 进入容器
docker compose exec openmemory-api bash

# 打开数据库
sqlite3 openmemory.db

# 查看所有记忆的衰退分数
SELECT 
    id,
    content,
    decay_score,
    importance_score,
    access_count,
    last_accessed_at,
    created_at
FROM memories 
WHERE state = 'active'
ORDER BY decay_score ASC
LIMIT 20;

# 查看衰退分数统计
SELECT 
    CASE 
        WHEN decay_score >= 0.7 THEN '新鲜 (≥0.7)'
        WHEN decay_score >= 0.3 THEN '中等 (0.3-0.7)'
        ELSE '衰退 (<0.3)'
    END as decay_level,
    COUNT(*) as count,
    AVG(decay_score) as avg_score
FROM memories 
WHERE state = 'active'
GROUP BY decay_level;

# 退出
.exit
exit
```

### 方法2：使用 MySQL 客户端（如果使用 MySQL）

```bash
# 进入容器
docker compose exec openmemory-api bash

# 连接数据库
mysql -u username -p openmemory

# 查看衰退分数
SELECT 
    id,
    SUBSTRING(content, 1, 50) as content_preview,
    decay_score,
    importance_score,
    access_count,
    last_accessed_at,
    created_at
FROM memories 
WHERE state = 'active'
ORDER BY decay_score ASC
LIMIT 20;

# 退出
exit;
exit
```

### 方法3：使用数据库管理工具

**推荐工具：**
- **DBeaver**：https://dbeaver.io/
- **DB Browser for SQLite**：https://sqlitebrowser.org/
- **phpMyAdmin**（MySQL）

**连接信息：**
```
数据库类型：SQLite / MySQL
数据库文件：api/openmemory.db (SQLite)
主机：localhost
端口：3306 (MySQL)
```

---

## 🔌 通过 API 查看

### 1. 查看单个记忆的衰退分数

**接口：** `GET /api/v1/memories/{memory_id}`

```bash
curl "http://localhost:8765/api/v1/memories/abc123"
```

**当前响应：**
```json
{
  "id": "abc123",
  "text": "记忆内容",
  "created_at": 1733184000,
  "state": "active",
  "app_id": "app1",
  "app_name": "测试应用",
  "categories": ["工作"],
  "metadata_": {}
}
```

**需要添加的字段：**
```json
{
  "id": "abc123",
  "text": "记忆内容",
  "created_at": 1733184000,
  "state": "active",
  "app_id": "app1",
  "app_name": "测试应用",
  "categories": ["工作"],
  "metadata_": {},
  
  // 新增衰退相关字段
  "decay_score": 0.65,
  "importance_score": 0.5,
  "access_count": 5,
  "last_accessed_at": 1733270400
}
```

### 2. 查看记忆列表的衰退分数

**接口：** `GET /api/v1/memories/`

```bash
curl "http://localhost:8765/api/v1/memories/?user_id=user123&page=1&size=10"
```

**响应示例：**
```json
{
  "items": [
    {
      "id": "abc123",
      "content": "记忆内容",
      "decay_score": 0.85,
      "importance_score": 0.7,
      "access_count": 10,
      "created_at": "2025-12-01T10:00:00",
      "categories": ["工作"]
    }
  ],
  "total": 100,
  "page": 1,
  "size": 10
}
```

### 3. 查看衰退统计

**接口：** `GET /api/v1/decay/statistics?user_id=user123`

```bash
curl "http://localhost:8765/api/v1/decay/statistics?user_id=user123"
```

**响应：**
```json
{
  "success": true,
  "statistics": {
    "total_active_memories": 150,
    "total_archived_memories": 25,
    "average_decay_score": 0.65,
    "high_decay_count": 80,
    "medium_decay_count": 60,
    "low_decay_count": 10
  }
}
```

### 4. 查看衰退严重的记忆

**接口：** `GET /api/v1/decay/decayed-memories`

```bash
curl "http://localhost:8765/api/v1/decay/decayed-memories?user_id=user123&threshold=0.3&limit=50"
```

**响应：**
```json
{
  "success": true,
  "count": 10,
  "threshold": 0.3,
  "memories": [
    {
      "id": "abc123",
      "content": "记忆内容...",
      "decay_score": 0.25,
      "importance_score": 0.5,
      "access_count": 2,
      "last_accessed_at": "2025-11-01T10:00:00",
      "created_at": "2025-10-01T10:00:00"
    }
  ]
}
```

---

## 🌐 在 Web 界面查看

### 当前状态

目前 Web 界面（`http://localhost:3000`）**不显示**衰退分数。

### 需要修改的前端文件

要在 Web 界面显示衰退分数，需要修改以下文件：

#### 1. 记忆列表组件

**文件位置：** `ui/components/MemoryList.tsx` 或类似文件

**需要添加的显示：**
```tsx
<div className="memory-card">
  <div className="memory-content">{memory.content}</div>
  
  {/* 新增：衰退分数显示 */}
  <div className="memory-meta">
    <span className="decay-score">
      衰退分数: {(memory.decay_score * 100).toFixed(0)}%
    </span>
    <span className={`decay-badge ${getDecayLevel(memory.decay_score)}`}>
      {getDecayLabel(memory.decay_score)}
    </span>
  </div>
</div>
```

**辅助函数：**
```typescript
function getDecayLevel(score: number): string {
  if (score >= 0.7) return 'high';
  if (score >= 0.3) return 'medium';
  return 'low';
}

function getDecayLabel(score: number): string {
  if (score >= 0.7) return '新鲜';
  if (score >= 0.3) return '中等';
  return '衰退';
}
```

**样式：**
```css
.decay-score {
  font-size: 0.9em;
  color: #666;
}

.decay-badge {
  padding: 2px 8px;
  border-radius: 4px;
  font-size: 0.8em;
  font-weight: bold;
}

.decay-badge.high {
  background-color: #4caf50;
  color: white;
}

.decay-badge.medium {
  background-color: #ff9800;
  color: white;
}

.decay-badge.low {
  background-color: #f44336;
  color: white;
}
```

#### 2. 记忆详情页面

**显示完整的衰退信息：**
```tsx
<div className="memory-detail">
  <h2>记忆详情</h2>
  
  <div className="detail-section">
    <h3>衰退信息</h3>
    <div className="decay-info">
      <div className="info-item">
        <label>衰退分数：</label>
        <span className="value">{(memory.decay_score * 100).toFixed(1)}%</span>
        <div className="progress-bar">
          <div 
            className="progress-fill" 
            style={{width: `${memory.decay_score * 100}%`}}
          />
        </div>
      </div>
      
      <div className="info-item">
        <label>重要性：</label>
        <span className="value">{(memory.importance_score * 100).toFixed(0)}%</span>
      </div>
      
      <div className="info-item">
        <label>访问次数：</label>
        <span className="value">{memory.access_count} 次</span>
      </div>
      
      <div className="info-item">
        <label>最后访问：</label>
        <span className="value">
          {memory.last_accessed_at 
            ? new Date(memory.last_accessed_at).toLocaleString() 
            : '从未访问'}
        </span>
      </div>
    </div>
  </div>
</div>
```

#### 3. 统计仪表板

**添加衰退统计图表：**
```tsx
<div className="dashboard">
  <h2>记忆统计</h2>
  
  <div className="stats-grid">
    <div className="stat-card">
      <h3>平均衰退分数</h3>
      <div className="stat-value">
        {(stats.average_decay_score * 100).toFixed(1)}%
      </div>
    </div>
    
    <div className="stat-card">
      <h3>记忆分布</h3>
      <div className="distribution">
        <div className="dist-item">
          <span className="label">新鲜</span>
          <span className="count">{stats.high_decay_count}</span>
        </div>
        <div className="dist-item">
          <span className="label">中等</span>
          <span className="count">{stats.medium_decay_count}</span>
        </div>
        <div className="dist-item">
          <span className="label">衰退</span>
          <span className="count">{stats.low_decay_count}</span>
        </div>
      </div>
    </div>
  </div>
</div>
```

---

## 📊 可视化示例

### 衰退分数进度条

```
新鲜记忆 (0.85):
████████████████████░░░░  85%

中等衰退 (0.45):
█████████░░░░░░░░░░░░░░░  45%

严重衰退 (0.12):
██░░░░░░░░░░░░░░░░░░░░░░  12%
```

### 衰退等级徽章

```
🟢 新鲜 (≥0.7)
🟡 中等 (0.3-0.7)
🔴 衰退 (<0.3)
```

---

## 🔧 快速查看命令

### 查看衰退分数最低的10条记忆

```bash
docker compose exec openmemory-api sqlite3 openmemory.db \
  "SELECT id, SUBSTR(content, 1, 50), decay_score 
   FROM memories 
   WHERE state='active' 
   ORDER BY decay_score ASC 
   LIMIT 10;"
```

### 查看衰退分数分布

```bash
docker compose exec openmemory-api sqlite3 openmemory.db \
  "SELECT 
     CASE 
       WHEN decay_score >= 0.7 THEN '新鲜'
       WHEN decay_score >= 0.3 THEN '中等'
       ELSE '衰退'
     END as level,
     COUNT(*) as count
   FROM memories 
   WHERE state='active'
   GROUP BY level;"
```

### 查看特定用户的衰退统计

```bash
curl "http://localhost:8765/api/v1/decay/statistics?user_id=user123" | jq
```

---

## 📱 移动端/桌面端显示建议

### 列表视图（简洁）

```
┌─────────────────────────────────┐
│ 记忆内容...                      │
│ 🟢 85% | 访问: 10次 | 2天前     │
└─────────────────────────────────┘
```

### 详情视图（完整）

```
┌─────────────────────────────────┐
│ 记忆详情                         │
├─────────────────────────────────┤
│ 衰退分数                         │
│ ████████████████░░░░ 85%        │
│                                  │
│ 重要性: ⭐⭐⭐⭐⭐ (100%)        │
│ 访问次数: 10次                   │
│ 最后访问: 2天前                  │
│ 创建时间: 30天前                 │
└─────────────────────────────────┘
```

---

## ✅ 总结

### 查看衰退分数的方式

1. **数据库直接查询**
   - SQLite 命令行
   - MySQL 客户端
   - 数据库管理工具

2. **API 接口**
   - 单个记忆详情
   - 记忆列表
   - 衰退统计
   - 衰退记忆列表

3. **Web 界面**（需要前端修改）
   - 记忆列表显示
   - 详情页面显示
   - 统计仪表板

### 衰退分数字段

- `decay_score`: 衰退分数 (0.0-1.0)
- `importance_score`: 重要性分数 (0.0-1.0)
- `access_count`: 访问次数
- `last_accessed_at`: 最后访问时间

### 快速命令

```bash
# 查看数据库中的衰退分数
docker compose exec openmemory-api sqlite3 openmemory.db \
  "SELECT id, decay_score, content FROM memories LIMIT 10;"

# 通过API查看统计
curl "http://localhost:8765/api/v1/decay/statistics?user_id=user123"

# 查看衰退记忆
curl "http://localhost:8765/api/v1/decay/decayed-memories?user_id=user123&threshold=0.3"
```

**衰退分数已经存储在数据库中，可以通过多种方式查看！** 📊