# 衰退分数显示测试指南 🧪

## 📋 测试概述

本指南帮助您快速验证衰退分数功能是否正常工作，包括数据库、API和Web界面的测试。

---

## 🔧 前置准备

### 1. 确保数据库迁移已完成

```bash
# 进入API容器
docker compose exec openmemory-api bash

# 运行数据库迁移
alembic upgrade head

# 验证迁移成功
alembic current

# 退出容器
exit
```

### 2. 重启服务

```bash
# 重启所有服务
docker compose restart

# 查看日志确认启动成功
docker compose logs -f openmemory-api
```

---

## 🗄️ 数据库测试

### 测试1：检查表结构

```bash
# 进入容器
docker compose exec openmemory-api bash

# 打开数据库
sqlite3 openmemory.db

# 查看memories表结构
.schema memories

# 应该看到以下字段：
# - decay_score FLOAT DEFAULT 1.0
# - importance_score FLOAT DEFAULT 0.5
# - access_count INTEGER DEFAULT 0
# - last_accessed_at TIMESTAMP

# 退出
.exit
exit
```

**预期结果：** ✅ 表中包含所有衰退相关字段

---

### 测试2：查看现有记忆的衰退分数

```bash
docker compose exec openmemory-api sqlite3 openmemory.db \
  "SELECT id, SUBSTR(content, 1, 30) as content, decay_score, importance_score, access_count 
   FROM memories 
   WHERE state='active' 
   LIMIT 5;"
```

**预期结果：** ✅ 显示记忆列表及其衰退分数

**示例输出：**
```
abc123|这是一条测试记忆...|1.0|0.5|0
def456|另一条记忆内容...|0.85|0.7|5
```

---

### 测试3：手动插入测试数据

```bash
docker compose exec openmemory-api sqlite3 openmemory.db <<EOF
-- 更新一条记忆的衰退分数用于测试
UPDATE memories 
SET decay_score = 0.3, 
    importance_score = 0.8, 
    access_count = 10,
    last_accessed_at = datetime('now')
WHERE id = (SELECT id FROM memories WHERE state='active' LIMIT 1);

-- 查看更新结果
SELECT id, decay_score, importance_score, access_count 
FROM memories 
WHERE state='active' 
LIMIT 1;
EOF
```

**预期结果：** ✅ 成功更新并显示新的衰退分数

---

## 🔌 API 测试

### 测试4：获取单个记忆（包含衰退分数）

```bash
# 首先获取一个记忆ID
MEMORY_ID=$(docker compose exec openmemory-api sqlite3 openmemory.db \
  "SELECT id FROM memories WHERE state='active' LIMIT 1;")

# 调用API获取记忆详情
curl -s "http://localhost:8765/api/v1/memories/${MEMORY_ID}" | jq
```

**预期结果：** ✅ 返回的JSON包含以下字段：
```json
{
  "id": "...",
  "text": "...",
  "decay_score": 0.85,
  "importance_score": 0.5,
  "access_count": 5,
  "last_accessed_at": 1733184000,
  ...
}
```

---

### 测试5：获取记忆列表（包含衰退分数）

```bash
# 获取记忆列表
curl -s "http://localhost:8765/api/v1/memories/?user_id=test_user&page=1&size=5" | jq '.items[0]'
```

**预期结果：** ✅ 列表中每条记忆都包含衰退字段

---

### 测试6：获取衰退统计

```bash
# 获取衰退统计信息
curl -s "http://localhost:8765/api/v1/decay/statistics?user_id=test_user" | jq
```

**预期结果：** ✅ 返回统计信息：
```json
{
  "success": true,
  "statistics": {
    "total_active_memories": 150,
    "total_archived_memories": 25,
    "average_decay_score": 0.65,
    "high_decay_count": 80,
    "medium_decay_count": 60,
    "low_decay_count": 10
  }
}
```

---

### 测试7：获取衰退记忆列表

```bash
# 获取衰退分数低于0.3的记忆
curl -s "http://localhost:8765/api/v1/decay/decayed-memories?user_id=test_user&threshold=0.3&limit=10" | jq
```

**预期结果：** ✅ 返回衰退记忆列表

---

### 测试8：手动触发衰退更新

```bash
# 触发衰退更新
curl -X POST "http://localhost:8765/api/v1/decay/trigger-update" \
  -H "Content-Type: application/json" \
  -d '{"user_id": "test_user"}' | jq
```

**预期结果：** ✅ 返回更新成功信息：
```json
{
  "success": true,
  "message": "Decay scores updated successfully",
  "updated_count": 150
}
```

---

### 测试9：更新记忆重要性

```bash
# 获取一个记忆ID
MEMORY_ID=$(docker compose exec openmemory-api sqlite3 openmemory.db \
  "SELECT id FROM memories WHERE state='active' LIMIT 1;")

# 更新重要性分数
curl -X POST "http://localhost:8765/api/v1/decay/update-importance" \
  -H "Content-Type: application/json" \
  -d "{
    \"memory_id\": \"${MEMORY_ID}\",
    \"importance_score\": 0.9
  }" | jq
```

**预期结果：** ✅ 返回更新成功信息

---

## 🌐 Web界面测试

### 测试10：浏览器开发者工具检查

1. 打开浏览器访问 `http://localhost:3000`
2. 打开开发者工具（F12）
3. 切换到 Network 标签
4. 刷新页面，查看记忆列表的API请求
5. 点击 `/api/v1/memories/` 请求
6. 查看 Response 标签

**预期结果：** ✅ 响应数据包含衰退字段：
```json
{
  "items": [
    {
      "id": "...",
      "content": "...",
      "decay_score": 0.85,
      "importance_score": 0.5,
      "access_count": 5,
      "last_accessed_at": 1733184000
    }
  ]
}
```

---

### 测试11：控制台日志检查

在浏览器控制台执行：

```javascript
// 获取记忆数据
fetch('/api/v1/memories/?user_id=test_user&page=1&size=1')
  .then(r => r.json())
  .then(data => {
    console.log('记忆数据:', data.items[0]);
    console.log('衰退分数:', data.items[0].decay_score);
    console.log('重要性:', data.items[0].importance_score);
    console.log('访问次数:', data.items[0].access_count);
  });
```

**预期结果：** ✅ 控制台显示所有衰退字段

---

## 🔄 完整流程测试

### 测试12：端到端测试

```bash
#!/bin/bash
# 保存为 test_decay_e2e.sh

echo "=== 端到端衰退功能测试 ==="

# 1. 创建测试记忆
echo "1. 创建测试记忆..."
RESPONSE=$(curl -s -X POST "http://localhost:8765/api/v1/memories/" \
  -H "Content-Type: application/json" \
  -d '{
    "user_id": "test_user",
    "text": "这是一条测试记忆，用于验证衰退功能",
    "app": "test_app"
  }')

MEMORY_ID=$(echo $RESPONSE | jq -r '.id')
echo "✅ 记忆已创建: $MEMORY_ID"

# 2. 获取记忆详情
echo -e "\n2. 获取记忆详情..."
curl -s "http://localhost:8765/api/v1/memories/${MEMORY_ID}" | jq '{
  id: .id,
  decay_score: .decay_score,
  importance_score: .importance_score,
  access_count: .access_count
}'

# 3. 更新重要性
echo -e "\n3. 更新重要性为0.9..."
curl -s -X POST "http://localhost:8765/api/v1/decay/update-importance" \
  -H "Content-Type: application/json" \
  -d "{
    \"memory_id\": \"${MEMORY_ID}\",
    \"importance_score\": 0.9
  }" | jq

# 4. 触发衰退更新
echo -e "\n4. 触发衰退更新..."
curl -s -X POST "http://localhost:8765/api/v1/decay/trigger-update" \
  -H "Content-Type: application/json" \
  -d '{"user_id": "test_user"}' | jq

# 5. 再次获取记忆详情
echo -e "\n5. 验证更新后的数据..."
curl -s "http://localhost:8765/api/v1/memories/${MEMORY_ID}" | jq '{
  id: .id,
  decay_score: .decay_score,
  importance_score: .importance_score,
  access_count: .access_count
}'

# 6. 获取统计信息
echo -e "\n6. 获取统计信息..."
curl -s "http://localhost:8765/api/v1/decay/statistics?user_id=test_user" | jq

echo -e "\n=== 测试完成 ==="
```

运行测试：
```bash
chmod +x test_decay_e2e.sh
./test_decay_e2e.sh
```

**预期结果：** ✅ 所有步骤成功执行，数据正确更新

---

## 📊 性能测试

### 测试13：批量衰退更新性能

```bash
# 测试批量更新性能
time curl -X POST "http://localhost:8765/api/v1/decay/trigger-update" \
  -H "Content-Type: application/json" \
  -d '{"user_id": "test_user"}'
```

**预期结果：** ✅ 更新时间合理（取决于记忆数量）

---

## 🐛 常见问题排查

### 问题1：API返回的数据不包含衰退字段

**检查步骤：**
```bash
# 1. 检查数据库迁移
docker compose exec openmemory-api alembic current

# 2. 检查表结构
docker compose exec openmemory-api sqlite3 openmemory.db ".schema memories"

# 3. 检查API代码
docker compose exec openmemory-api cat /app/app/schemas.py | grep decay_score
```

**解决方案：**
- 运行数据库迁移：`alembic upgrade head`
- 重启服务：`docker compose restart`

---

### 问题2：衰退分数始终为1.0

**检查步骤：**
```bash
# 检查定时任务是否运行
curl "http://localhost:8765/api/v1/decay/scheduler-status"

# 手动触发更新
curl -X POST "http://localhost:8765/api/v1/decay/trigger-update" \
  -H "Content-Type: application/json" \
  -d '{"user_id": "test_user"}'
```

**解决方案：**
- 确保环境变量 `MEMORY_DECAY_ENABLED=true`
- 检查日志：`docker compose logs openmemory-api | grep decay`

---

### 问题3：Web界面不显示衰退分数

**检查步骤：**
1. 打开浏览器开发者工具
2. 检查Network标签中的API响应
3. 检查Console标签中的错误信息

**解决方案：**
- 确认API返回包含衰退字段
- 检查前端组件是否正确渲染
- 清除浏览器缓存并刷新

---

## ✅ 测试检查清单

完成所有测试后，请确认：

- [ ] 数据库表包含衰退字段
- [ ] 可以在数据库中查询衰退分数
- [ ] API单个记忆接口返回衰退字段
- [ ] API记忆列表接口返回衰退字段
- [ ] 衰退统计接口正常工作
- [ ] 衰退记忆列表接口正常工作
- [ ] 手动触发更新功能正常
- [ ] 更新重要性功能正常
- [ ] 浏览器Network显示衰退字段
- [ ] 定时任务正常运行
- [ ] 端到端测试通过
- [ ] 性能测试通过

---

## 📈 测试报告模板

```markdown
# 衰退分数功能测试报告

**测试日期：** 2025-12-03
**测试人员：** [您的名字]
**环境：** Docker / 本地开发

## 测试结果

| 测试项 | 状态 | 备注 |
|--------|------|------|
| 数据库结构 | ✅ | 所有字段正常 |
| API单个记忆 | ✅ | 返回衰退字段 |
| API记忆列表 | ✅ | 返回衰退字段 |
| 衰退统计 | ✅ | 数据准确 |
| 手动更新 | ✅ | 功能正常 |
| 定时任务 | ✅ | 按时执行 |
| Web界面 | ⚠️ | 需要前端修改 |

## 问题记录

1. [问题描述]
   - 解决方案：[...]
   - 状态：已解决/待解决

## 总结

衰退分数功能基本正常，API层面完全可用。
Web界面需要按照《前端显示衰退分数指南.md》进行修改。
```

---

## 🎯 快速验证命令

```bash
# 一键验证所有功能
echo "=== 快速验证 ==="
echo "1. 数据库字段："
docker compose exec openmemory-api sqlite3 openmemory.db \
  "SELECT decay_score, importance_score FROM memories LIMIT 1;"

echo -e "\n2. API返回："
curl -s "http://localhost:8765/api/v1/memories/?user_id=test_user&page=1&size=1" \
  | jq '.items[0] | {decay_score, importance_score, access_count}'

echo -e "\n3. 统计信息："
curl -s "http://localhost:8765/api/v1/decay/statistics?user_id=test_user" \
  | jq '.statistics | {average_decay_score, high_decay_count}'

echo -e "\n=== 验证完成 ==="
```

---

**按照本指南完成测试，确保衰退分数功能正常工作！** 🧪✅