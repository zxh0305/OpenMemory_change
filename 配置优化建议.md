# OpenMemory 配置优化建议 🚀

> 根据不同使用场景优化 OpenMemory 的性能和资源占用

## 📊 使用场景分类

### 1️⃣ 个人使用 (< 10,000 条记忆)

**推荐配置**: 最小化资源占用

#### api/.env
```env
# 基础配置
OPENAI_API_KEY=sk-xxx
USER=my_user

# 使用轻量级模型
OPENAI_MODEL=gpt-4o-mini
CATEGORIZATION_OPENAI_MODEL=gpt-4o-mini

# 使用较小的嵌入维度
OPENAI_EMBEDDING_MODEL=text-embedding-3-small
OPENAI_EMBEDDING_MODEL_DIMS=512  # 降低到 512 维

# 使用 SQLite (默认)
DATABASE_URL=sqlite:///./openmemory.db
```

**资源需求**:
- 内存: 1.5-2 GB
- 磁盘: 1-2 GB
- CPU: 2 核心

**优势**:
- ✅ 启动快速
- ✅ 资源占用少
- ✅ 配置简单

---

### 2️⃣ 团队使用 (10,000 - 100,000 条记忆)

**推荐配置**: 平衡性能和资源

#### api/.env
```env
# 基础配置
OPENAI_API_KEY=sk-xxx

# 使用标准模型
OPENAI_MODEL=gpt-4o-mini
OPENAI_EMBEDDING_MODEL=text-embedding-3-small
OPENAI_EMBEDDING_MODEL_DIMS=1536  # 标准维度

# 使用 PostgreSQL
DATABASE_URL=postgresql://openmemory:password@localhost:5432/openmemory
```

#### docker-compose.yml 添加 PostgreSQL
```yaml
services:
  postgres:
    image: postgres:15
    container_name: openmemory-postgres
    environment:
      POSTGRES_DB: openmemory
      POSTGRES_USER: openmemory
      POSTGRES_PASSWORD: password
    ports:
      - "5432:5432"
    volumes:
      - postgres_data:/var/lib/postgresql/data

  openmemory-api:
    depends_on:
      - postgres
      - mem0_store
    # ... 其他配置

volumes:
  postgres_data:
```

**资源需求**:
- 内存: 4-6 GB
- 磁盘: 10-20 GB
- CPU: 4 核心

**优势**:
- ✅ 更好的并发性能
- ✅ 支持更多用户
- ✅ 数据更安全

---

### 3️⃣ 企业使用 (> 100,000 条记忆)

**推荐配置**: 最大化性能

#### api/.env
```env
# 基础配置
OPENAI_API_KEY=sk-xxx

# 使用高性能模型
OPENAI_MODEL=gpt-4o
OPENAI_EMBEDDING_MODEL=text-embedding-3-large
OPENAI_EMBEDDING_MODEL_DIMS=3072  # 最大维度

# 使用 PostgreSQL 集群
DATABASE_URL=postgresql://openmemory:password@postgres-primary:5432/openmemory
```

#### docker-compose.yml 资源限制
```yaml
services:
  mem0_store:
    deploy:
      resources:
        limits:
          memory: 8G
          cpus: '4'
        reservations:
          memory: 4G
          cpus: '2'

  openmemory-api:
    command: uvicorn main:app --host 0.0.0.0 --port 8765 --workers 8
    deploy:
      resources:
        limits:
          memory: 4G
          cpus: '4'
        reservations:
          memory: 2G
          cpus: '2'
```

**资源需求**:
- 内存: 16+ GB
- 磁盘: 100+ GB SSD
- CPU: 8+ 核心

**优势**:
- ✅ 最佳性能
- ✅ 支持大规模数据
- ✅ 高可用性

---

## 🌐 国内 API 服务配置

### 硅基流动 (SiliconFlow)

```env
# 高性价比配置
OPENAI_API_KEY=sk-your-siliconflow-key
OPENAI_PROVIDER=openai_structured
OPENAI_BASE_URL=https://api.siliconflow.cn/v1
OPENAI_MODEL=deepseek-ai/DeepSeek-R1-0528-Qwen3-8B

# 嵌入模型
OPENAI_EMBEDDING_MODEL_API_KEY=sk-your-siliconflow-key
OPENAI_EMBEDDING_MODEL_BASE_URL=https://api.siliconflow.cn/v1
OPENAI_EMBEDDING_MODEL=Qwen/Qwen3-Embedding-4B
OPENAI_EMBEDDING_MODEL_DIMS=1024

# 分类功能
CATEGORIZATION_OPENAI_API_KEY=sk-your-siliconflow-key
CATEGORIZATION_OPENAI_BASE_URL=https://api.siliconflow.cn/v1
CATEGORIZATION_OPENAI_MODEL=deepseek-ai/DeepSeek-R1-0528-Qwen3-8B
```

**优势**:
- 💰 价格便宜 (约为 OpenAI 的 1/10)
- 🚀 国内访问快
- 🇨🇳 支持中文优化

### DeepSeek

```env
OPENAI_API_KEY=sk-your-deepseek-key
OPENAI_BASE_URL=https://api.deepseek.com/v1
OPENAI_MODEL=deepseek-chat

OPENAI_EMBEDDING_MODEL_API_KEY=sk-your-deepseek-key
OPENAI_EMBEDDING_MODEL_BASE_URL=https://api.deepseek.com/v1
OPENAI_EMBEDDING_MODEL=deepseek-embedding
```

### 智谱 AI (GLM)

```env
OPENAI_API_KEY=your-zhipu-key
OPENAI_BASE_URL=https://open.bigmodel.cn/api/paas/v4
OPENAI_MODEL=glm-4-flash

OPENAI_EMBEDDING_MODEL_API_KEY=your-zhipu-key
OPENAI_EMBEDDING_MODEL_BASE_URL=https://open.bigmodel.cn/api/paas/v4
OPENAI_EMBEDDING_MODEL=embedding-2
```

---

## ⚡ 性能优化技巧

### 1. 减少向量维度

**效果**: 内存占用减少 66%，搜索速度提升 50%

```env
# 从 1536 降到 512
OPENAI_EMBEDDING_MODEL_DIMS=512
```

**适用场景**: 个人使用，对搜索精度要求不高

### 2. 禁用自动分类

**效果**: 减少 API 调用，加快记忆保存速度

修改 `api/app/models.py`:
```python
# 注释掉这两个事件监听器
# @event.listens_for(Memory, "after_insert")
# def after_memory_insert(mapper, connection, target):
#     ...

# @event.listens_for(Memory, "after_update")
# def after_memory_update(mapper, connection, target):
#     ...
```

**适用场景**: 不需要自动分类功能

### 3. 调整工作进程数

```yaml
# docker-compose.yml
command: uvicorn main:app --workers 4
# 推荐: workers = CPU 核心数 × 2
```

### 4. 使用 Redis 缓存

添加 Redis 服务:
```yaml
services:
  redis:
    image: redis:7-alpine
    container_name: openmemory-redis
    ports:
      - "6379:6379"
```

### 5. 启用 Gzip 压缩

在 `api/main.py` 中添加:
```python
from fastapi.middleware.gzip import GZipMiddleware

app.add_middleware(GZipMiddleware, minimum_size=1000)
```

---

## 💾 存储优化

### 1. 定期清理已删除记忆

```sql
-- 永久删除标记为 deleted 的记忆
DELETE FROM memories WHERE state = 'deleted' AND deleted_at < NOW() - INTERVAL '30 days';
```

### 2. 归档旧记忆

```sql
-- 将 6 个月前的记忆归档
UPDATE memories 
SET state = 'archived', archived_at = NOW()
WHERE created_at < NOW() - INTERVAL '6 months' 
AND state = 'active';
```

### 3. 压缩 Qdrant 数据

```bash
# 进入 Qdrant 容器
docker exec -it openmemory-qdrant sh

# 优化集合
curl -X POST 'http://localhost:6333/collections/openmemory/optimize'
```

---

## 🔒 安全优化

### 1. 使用环境变量

**不要在 .env 文件中存储生产密钥**

```bash
# 使用系统环境变量
export OPENAI_API_KEY=sk-xxx
export DATABASE_URL=postgresql://...

# 或使用 Docker secrets
docker secret create openai_key openai_key.txt
```

### 2. 限制 API 访问

使用 Nginx 反向代理:
```nginx
server {
    listen 80;
    server_name your-domain.com;

    # 限制请求速率
    limit_req_zone $binary_remote_addr zone=api:10m rate=10r/s;

    location /api {
        limit_req zone=api burst=20;
        proxy_pass http://localhost:8765;
    }
}
```

### 3. 启用 HTTPS

```bash
# 使用 Certbot
sudo certbot --nginx -d your-domain.com
```

---

## 📊 监控建议

### 1. 资源监控

```bash
# 实时监控
docker stats

# 查看磁盘使用
du -sh mem0_storage/
du -sh api/openmemory.db
```

### 2. 日志管理

```yaml
# docker-compose.yml
services:
  openmemory-api:
    logging:
      driver: "json-file"
      options:
        max-size: "10m"
        max-file: "3"
```

### 3. 健康检查

```yaml
healthcheck:
  test: ["CMD", "curl", "-f", "http://localhost:8765/health"]
  interval: 30s
  timeout: 10s
  retries: 3
```

---

## 🎯 推荐配置组合

### 最小配置 (个人)
```env
OPENAI_API_KEY=sk-xxx
OPENAI_MODEL=gpt-4o-mini
OPENAI_EMBEDDING_MODEL_DIMS=512
DATABASE_URL=sqlite:///./openmemory.db
```

### 平衡配置 (团队)
```env
OPENAI_API_KEY=sk-xxx
OPENAI_MODEL=gpt-4o-mini
OPENAI_EMBEDDING_MODEL_DIMS=1536
DATABASE_URL=postgresql://user:pass@localhost:5432/openmemory
```

### 高性能配置 (企业)
```env
OPENAI_API_KEY=sk-xxx
OPENAI_MODEL=gpt-4o
OPENAI_EMBEDDING_MODEL=text-embedding-3-large
OPENAI_EMBEDDING_MODEL_DIMS=3072
DATABASE_URL=postgresql://user:pass@postgres-cluster:5432/openmemory
```

### 国内优化配置
```env
OPENAI_API_KEY=sk-siliconflow-key
OPENAI_PROVIDER=openai_structured
OPENAI_BASE_URL=https://api.siliconflow.cn/v1
OPENAI_MODEL=deepseek-ai/DeepSeek-R1
OPENAI_EMBEDDING_MODEL=Qwen/Qwen3-Embedding-4B
OPENAI_EMBEDDING_MODEL_DIMS=1024
```

---

## 📈 性能对比

| 配置 | 内存 | 响应时间 | 成本/月 | 适用场景 |
|------|------|----------|---------|----------|
| 最小 | 2GB | ~500ms | $5-10 | 个人 |
| 平衡 | 4GB | ~300ms | $20-50 | 团队 |
| 高性能 | 16GB | ~100ms | $100+ | 企业 |
| 国内 | 2-4GB | ~200ms | $2-5 | 国内用户 |

---

## ✅ 优化检查清单

- [ ] 选择合适的 LLM 模型
- [ ] 调整嵌入向量维度
- [ ] 配置合适的数据库
- [ ] 设置资源限制
- [ ] 启用日志管理
- [ ] 配置健康检查
- [ ] 设置备份策略
- [ ] 启用 HTTPS (生产环境)
- [ ] 配置监控告警
- [ ] 定期清理数据

---

**需要帮助?** 查看 [详细使用指南](README_详细使用指南.md) 或 [快速开始](快速开始.md)