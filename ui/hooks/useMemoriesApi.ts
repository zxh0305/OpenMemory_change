...
interface ApiMemoryItem {
  id: string;
  content: string;
  created_at: number | string; // Backend returns int (timestamp), but can also be string
  state: string;
  app_id: string;
  categories: string[];
  ...
  const fetchMemories = useCallback(async (
    ...
    try {
      ...
      const adaptedMemories: Memory[] = response.data.items.map((item: ApiMemoryItem) => {
        // Handle created_at: backend returns int (timestamp) or string
        let created_at_timestamp: number;
        if (typeof item.created_at === 'number') {
          // Already a timestamp (in seconds), convert to milliseconds
          created_at_timestamp = item.created_at * 1000;
        } else {
          // String format, parse it
          created_at_timestamp = new Date(item.created_at).getTime();
        }
