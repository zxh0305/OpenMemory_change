...
export const Install = () => {
  ...
  // 从 Redux store 获取当前用户 ID，这样切换用户时会自动更新
  const currentUserId = useSelector((state: RootState) => state.profile.userId);
  
  // 确保用户 ID 从 Redux store 获取，如果为空则从 localStorage 读取（用于 SSR 兼容）
  const [user, setUser] = useState<string>("user");
  
  useEffect(() => {
    // 优先使用 Redux store 中的用户 ID
    if (currentUserId) {
      setUser(currentUserId);
    } else if (typeof window !== 'undefined') {
      // 如果 Redux store 中没有，从 localStorage 读取
      const storedUserId = localStorage.getItem('openmemory_user_id');
      if (storedUserId) {
        setUser(storedUserId);
      } else {
        // 最后使用环境变量或默认值
        setUser(process.env.NEXT_PUBLIC_USER_ID || "user");
      }
    }
  }, [currentUserId]);
