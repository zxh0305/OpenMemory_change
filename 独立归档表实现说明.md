# 独立归档表实现说明 📦

## 🎯 设计目标

将衰退后的记忆存储在**独立的归档表** `archived_memories` 中，与活跃记忆表 `memories` 完全分离。

---

## 📊 表结构设计

### 1. 活跃记忆表 (memories)
存储所有活跃状态的记忆

```sql
CREATE TABLE memories (
    id VARCHAR(32) PRIMARY KEY,
    user_id VARCHAR(32) NOT NULL,
    app_id VARCHAR(32) NOT NULL,
    content TEXT NOT NULL,
    vector TEXT,
    metadata JSON,
    state ENUM('active', 'paused', 'deleted'),  -- 不包含 archived
    created_at DATETIME NOT NULL,
    updated_at DATETIME NOT NULL,
    
    -- 衰退相关字段
    decay_score FLOAT DEFAULT 1.0,
    last_accessed_at DATETIME,
    access_count INT DEFAULT 0,
    importance_score FLOAT DEFAULT 0.5,
    
    INDEX idx_memory_user_state (user_id, state),
    INDEX idx_memory_decay_score (decay_score)
);
```

### 2. 归档记忆表 (archived_memories)
**独立存储**衰退后的记忆

```sql
CREATE TABLE archived_memories (
    id VARCHAR(32) PRIMARY KEY,  -- 保持原记忆ID
    user_id VARCHAR(32) NOT NULL,
    app_id VARCHAR(32) NOT NULL,
    content TEXT NOT NULL,
    vector TEXT,
    metadata JSON,
    
    -- 原始时间信息
    created_at DATETIME NOT NULL,  -- 原始创建时间
    updated_at DATETIME NOT NULL,  -- 原始更新时间
    
    -- 归档信息
    archived_at DATETIME NOT NULL,  -- 归档时间
    archived_from_state ENUM('active', 'paused'),  -- 归档前状态
    
    -- 衰退信息快照
    decay_score_at_archive FLOAT NOT NULL,  -- 归档时的衰退分数
    last_accessed_at DATETIME,
    access_count INT DEFAULT 0,
    importance_score FLOAT DEFAULT 0.5,
    
    -- 分类快照（JSON存储）
    categories_snapshot JSON,
    
    INDEX idx_archived_user_time (user_id, archived_at),
    INDEX idx_archived_decay (decay_score_at_archive)
);
```

---

## 🔄 记忆流转过程

### 完整生命周期

```
┌─────────────────────────────────────────────────────────┐
│                    创建新记忆                            │
│                       ↓                                  │
│              ┌──────────────┐                           │
│              │ memories 表  │                           │
│              │ state=active │                           │
│              │ decay=1.0    │                           │
│              └──────┬───────┘                           │
│                     │                                    │
│              时间推移，未访问                            │
│                     │                                    │
│              ┌──────▼───────┐                           │
│              │ memories 表  │                           │
│              │ state=active │                           │
│              │ decay=0.5    │                           │
│              └──────┬───────┘                           │
│                     │                                    │
│         decay_score < 0.1 (阈值)                        │
│                     │                                    │
│              ┌──────▼───────┐                           │
│              │  移动操作    │                           │
│              │  1. 复制数据 │                           │
│              │  2. 删除原记录│                          │
│              └──────┬───────┘                           │
│                     │                                    │
│              ┌──────▼──────────────┐                    │
│              │ archived_memories 表│                    │
│              │ 独立存储            │                    │
│              │ decay=0.08          │                    │
│              └──────┬──────────────┘                    │
│                     │                                    │
│              用户请求恢复                                │
│                     │                                    │
│              ┌──────▼───────┐                           │
│              │  恢复操作    │                           │
│              │  1. 复制回去 │                           │
│              │  2. 删除归档 │                           │
│              └──────┬───────┘                           │
│                     │                                    │
│              ┌──────▼───────┐                           │
│              │ memories 表  │                           │
│              │ state=active │                           │
│              │ decay=1.0    │ (重置)                    │
│              └──────────────┘                           │
└─────────────────────────────────────────────────────────┘
```

---

## 💻 核心实现

### 1. 归档操作（移动到归档表）

文件：[`api/app/utils/decay_with_archive_table.py`](api/app/utils/decay_with_archive_table.py:119)

```python
def move_memory_to_archive(db: Session, memory: Memory) -> ArchivedMemory:
    """
    将记忆从 memories 表移动到 archived_memories 表
    """
    # 1. 获取分类信息
    categories = [cat.name for cat in memory.categories]
    
    # 2. 创建归档记录
    archived_memory = ArchivedMemory(
        id=memory.id,  # 保持相同ID
        user_id=memory.user_id,
        app_id=memory.app_id,
        content=memory.content,
        # ... 其他字段
        categories_snapshot=categories  # 保存分类快照
    )
    db.add(archived_memory)
    
    # 3. 记录状态变更历史
    history = MemoryStatusHistory(...)
    db.add(history)
    
    # 4. 删除原记录
    db.delete(memory)
    
    db.commit()
    return archived_memory
```

### 2. 恢复操作（移回活跃表）

```python
def restore_archived_memory(db: Session, memory_id: str, user_id: str) -> bool:
    """
    从归档表恢复记忆到活跃表
    """
    # 1. 查找归档记忆
    archived_memory = db.query(ArchivedMemory).filter(...).first()
    
    # 2. 创建新的活跃记忆
    memory = Memory(
        id=archived_memory.id,
        # ... 复制所有字段
        decay_score=1.0,  # 重置衰退分数
        state=MemoryState.active
    )
    db.add(memory)
    
    # 3. 恢复分类关联
    for cat_name in archived_memory.categories_snapshot:
        category = db.query(Category).filter(...).first()
        memory.categories.append(category)
    
    # 4. 删除归档记录
    db.delete(archived_memory)
    
    db.commit()
    return True
```

---

## 🔌 API 接口

### 1. 查询归档记忆列表

```bash
GET /api/v1/archived-memories/?user_id=user123&limit=50&offset=0
```

**响应：**
```json
[
  {
    "id": "abc123",
    "content": "记忆内容...",
    "created_at": "2025-10-01T10:00:00",
    "archived_at": "2025-12-01T02:00:00",
    "decay_score_at_archive": 0.08,
    "importance_score": 0.5,
    "access_count": 2,
    "categories": ["工作", "项目"]
  }
]
```

### 2. 获取单个归档记忆详情

```bash
GET /api/v1/archived-memories/abc123?user_id=user123
```

### 3. 恢复归档记忆

```bash
POST /api/v1/archived-memories/restore
Content-Type: application/json

{
  "memory_id": "abc123",
  "user_id": "user123"
}
```

**响应：**
```json
{
  "success": true,
  "message": "记忆已从归档表恢复到活跃表",
  "memory_id": "abc123",
  "details": {
    "from_table": "archived_memories",
    "to_table": "memories",
    "new_decay_score": 1.0
  }
}
```

### 4. 永久删除归档记忆

```bash
DELETE /api/v1/archived-memories/abc123?user_id=user123
```

**警告：此操作不可恢复！**

### 5. 归档统计信息

```bash
GET /api/v1/archived-memories/stats/summary?user_id=user123
```

**响应：**
```json
{
  "success": true,
  "total_archived": 25,
  "by_decay_score": {
    "very_low": 15,
    "low": 8,
    "medium": 2
  },
  "by_app": [
    {"app_id": "app1", "count": 10},
    {"app_id": "app2", "count": 15}
  ]
}
```

---

## 📁 文件清单

### 新增文件

1. ✅ [`api/app/models.py`](api/app/models.py:139) - 添加 `ArchivedMemory` 模型
2. ✅ [`api/alembic/versions/create_archived_memories_table.py`](api/alembic/versions/create_archived_memories_table.py) - 创建归档表迁移
3. ✅ [`api/app/utils/decay_with_archive_table.py`](api/app/utils/decay_with_archive_table.py) - 独立归档表的衰退逻辑
4. ✅ [`api/app/routers/archived_memories.py`](api/app/routers/archived_memories.py) - 归档记忆API路由
5. ✅ [`api/main.py`](api/main.py:8) - 集成归档记忆路由

---

## 🚀 部署步骤

### 1. 运行数据库迁移

```bash
# 进入容器
docker compose exec openmemory-api bash

# 运行迁移（按顺序）
alembic upgrade add_memory_decay_fields
alembic upgrade create_archived_memories_table

# 退出
exit
```

### 2. 重启服务

```bash
docker compose restart openmemory-api
```

### 3. 验证部署

```bash
# 检查归档表是否创建
docker compose exec openmemory-api bash
sqlite3 openmemory.db "SELECT name FROM sqlite_master WHERE type='table' AND name='archived_memories';"
```

---

## 🔍 查询示例

### 查看两个表的记忆分布

```sql
-- 活跃记忆统计
SELECT 
    state,
    COUNT(*) as count,
    AVG(decay_score) as avg_decay
FROM memories 
GROUP BY state;

-- 归档记忆统计
SELECT 
    COUNT(*) as total_archived,
    AVG(decay_score_at_archive) as avg_decay_at_archive,
    MIN(archived_at) as first_archived,
    MAX(archived_at) as last_archived
FROM archived_memories;
```

### 查看特定用户的记忆分布

```sql
-- 用户的活跃记忆
SELECT COUNT(*) FROM memories 
WHERE user_id = 'user123' AND state = 'active';

-- 用户的归档记忆
SELECT COUNT(*) FROM archived_memories 
WHERE user_id = 'user123';
```

---

## 💡 优势对比

### 独立归档表 vs 同表状态标记

| 特性 | 独立归档表 | 同表状态标记 |
|------|-----------|-------------|
| **查询性能** | ✅ 优秀（表更小） | ⚠️ 一般（需过滤） |
| **数据隔离** | ✅ 完全隔离 | ❌ 混在一起 |
| **索引效率** | ✅ 高效 | ⚠️ 索引包含归档数据 |
| **备份策略** | ✅ 可分别备份 | ❌ 必须一起备份 |
| **清理操作** | ✅ 直接删除表 | ⚠️ 需要WHERE过滤 |
| **存储优化** | ✅ 可用不同存储 | ❌ 同一存储 |
| **实现复杂度** | ⚠️ 稍复杂 | ✅ 简单 |

---

## 📊 性能优化

### 1. 索引策略

```sql
-- 活跃表索引（优化常用查询）
CREATE INDEX idx_memory_user_state ON memories(user_id, state);
CREATE INDEX idx_memory_decay_score ON memories(decay_score);

-- 归档表索引（优化归档查询）
CREATE INDEX idx_archived_user_time ON archived_memories(user_id, archived_at);
CREATE INDEX idx_archived_decay ON archived_memories(decay_score_at_archive);
```

### 2. 分区策略（可选）

对于大规模数据，可以对归档表按时间分区：

```sql
-- 按月分区归档表
CREATE TABLE archived_memories_2025_01 PARTITION OF archived_memories
FOR VALUES FROM ('2025-01-01') TO ('2025-02-01');
```

### 3. 归档数据压缩

```sql
-- 对归档表启用压缩（MySQL）
ALTER TABLE archived_memories ROW_FORMAT=COMPRESSED;
```

---

## 🎯 使用建议

### 1. 定期清理策略

```python
# 删除1年前的归档记忆
DELETE FROM archived_memories 
WHERE archived_at < NOW() - INTERVAL '1 year';
```

### 2. 批量恢复

```python
# 恢复重要的归档记忆
SELECT id FROM archived_memories 
WHERE importance_score > 0.8 
AND archived_at > NOW() - INTERVAL '30 days';
```

### 3. 监控指标

- 活跃表大小
- 归档表大小
- 每日归档数量
- 恢复操作频率

---

## ✅ 总结

### 核心特点

1. **完全分离**：活跃记忆和归档记忆存储在不同表
2. **数据完整**：归档时保存所有信息和快照
3. **可恢复性**：随时可以恢复到活跃表
4. **高性能**：查询活跃记忆更快
5. **易管理**：可以独立备份、清理归档数据

### 数据流向

```
memories 表 (活跃)
    ↓ 衰退严重
archived_memories 表 (归档)
    ↓ 用户恢复
memories 表 (活跃，重置衰退分数)
```

### 关键优势

✅ **性能优化**：活跃表更小，查询更快
✅ **数据隔离**：归档数据不影响日常操作
✅ **灵活管理**：可以独立处理归档数据
✅ **存储优化**：可以使用不同的存储策略
✅ **清晰架构**：数据流向清晰明确

**独立归档表是大规模记忆管理的最佳实践！** 🎉